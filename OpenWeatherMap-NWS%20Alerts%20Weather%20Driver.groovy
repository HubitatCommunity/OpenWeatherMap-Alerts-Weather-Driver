...
// >>>>>>>>>> End Process Standard Weather-Station Variables (Regardless of Forecast Selection)  <<<<<<<<<<	
        ...
        if(threedayTilePublish) {
            ...
            updateDataValue("imgName1", '<img class=\"centerImage\" src=' + getDataValue('iconLocation') + getImgName(owm.daily[1].weather[0].id, 'false') + (((getDataValue("iconLocation").toLowerCase().contains('://github.com/')) && (getDataValue("iconLocation").toLowerCase().contains('/blob/master/'))) ? "?raw=true" : "") + '>')
            updateDataValue("imgName2", '<img class=\"centerImage\" src=' + getDataValue('iconLocation') + getImgName(owm.daily[2].weather[0].id, 'false') + (((getDataValue("iconLocation").toLowerCase().contains('://github.com/')) && (getDataValue("iconLocation").toLowerCase().contains('/blob/master/'))) ? "?raw=true" : "") + '>')
        }
...
String getImgName(int wCode, String iconTOD){
...
void LOGDEBUG(txt){
    if(settings?.logSet == true){ log.debug("OpenWeatherMap.org Weather Driver - DEBUG:  ${txt}") }
    return
}

void LOGINFO(txt){
    if(settings?.logSet == true){log.info("OpenWeatherMap.org Weather Driver - INFO:  ${txt}") }
    return
}

void LOGWARN(txt){
    if(settings?.logSet == true){log.info("OpenWeatherMap.org Weather Driver - WARNING:  ${txt}") }
    return
}

void LOGERR(txt){
    if(settings?.logSet == true){log.info("OpenWeatherMap.org Weather Driver - ERROR:  ${txt}") }
    return
}
